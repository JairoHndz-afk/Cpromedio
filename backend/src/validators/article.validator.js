...
const ownedMediaUrlSchema = z
  .string()
  .refine((value) => isOwnedMediaUrl(value), "Solo se aceptan archivos del medio subidos a infraestructura controlada por la redacción.");
...
const articleEmbedSchema = z.object({
  ...
  title: z.string().max(160, "El título del embed no puede superar 160 caracteres.").optional().default("")
});
...
const articleContentBlockSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    text: z.string().min(1, "Cada párrafo debe incluir contenido.").max(4000, "Cada párrafo no puede superar 4000 caracteres.")
  }),
  z.object({
...
export const articleInputSchema = z
  .object({
    title: z.string().min(6, "El título debe tener al menos 6 caracteres.").max(180, "El título no puede superar 180 caracteres."),
    subtitle: z.string().max(220, "El subtítulo no puede superar 220 caracteres.").optional().default(""),
    ...
    contentBlocks: z.array(articleContentBlockSchema).max(120, "El artículo no puede superar 120 bloques.").optional().default([]),
    cover: z
      .object({
        url: ownedMediaUrlSchema.optional().or(z.literal("")).default(""),
        alt: z.string().max(140, "El texto alternativo de la portada no puede superar 140 caracteres.").optional().default(""),
        positionX: z.coerce.number().min(0, "La posición horizontal debe estar entre 0 y 100.").max(100, "La posición horizontal debe estar entre 0 y 100.").optional().default(50),
        positionY: z.coerce.number().min(0, "La posición vertical debe estar entre 0 y 100.").max(100, "La posición vertical debe estar entre 0 y 100.").optional().default(50),
  ...
  .refine((value) => Boolean(value.body) || value.contentBlocks.length > 0, {
    message: "Agrega al menos un bloque de contenido al artículo.",
    path: ["contentBlocks"]
  });
